...
interface PotholeDao {
    ...
    @Query("DELETE FROM pothole_data WHERE id IN (:ids)")
    suspend fun deleteByIds(ids: List<Long>)

    // Общее количество записей в базе
    @Query("SELECT COUNT(*) FROM pothole_data")
    suspend fun getCount(): Int

    // Количество отправленных записей (если есть флаг isSent)
    @Query("SELECT COUNT(*) FROM pothole_data WHERE isSent = 1")
    suspend fun getSentCount(): Int

    // Количество новых записей, которые ещё не отправлены
    @Query("SELECT COUNT(*) FROM pothole_data WHERE isSent = 0")
